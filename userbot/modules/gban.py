    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit(
                "`Terjadi Kesalahan... Mohon Lapor Ke` @IndomieGenetik", str(err)
            )
...
async def gben(userbot):
    ...
    return await dark.edit(
        ...
        f"**User ID :** `{user.id}`\n"
        f"**Action :** `Global Banned By` {me.first_name}(tg://user?id={me.id}) \n"
    )
...
async def gunben(userbot):
    ...
    try:
        if ungmute(user.id) is False:
            return await dark.edit(
                "**Kesalahan! Pengguna Tidak Di Global Banned.**"
            )
    except BaseException:
        pass
    return await dark.edit(
        ...
        f"**Pengguna :** [{user.first_name}](tg://user?id={user.id}) \n"
        f"**User ID :** `{user.id}`\n"
        f"**Action :** `UnGbanned By` {me.first_name}(tg://user?id={me.id}) \n"
    )
